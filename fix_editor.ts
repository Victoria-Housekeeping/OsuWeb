...
// The file has ~29KB of text.
// 1. Add File/Edit Menu inside TopMenu
const topMenuReplacement = `
  const TopMenu = () => (
...
content = content.replace(/const TopMenu = \(\) => \([^]*?  \);/, topMenuReplacement.trim());


// 2. Timeline navigation and timeline rendering
const timelineReplacement = `
  const renderTimeline = () => {
    ...
    const getLeft = (t: number) => \`\${((t - startTime) / viewWidthMs) * 100}%\`;
    ...
    return (
      ...
      >
        ...
        {ticks.map((tick, i) => {
          ...
          // red = 1/2 (index % 2 === 0)
          // blue = 1/4 (index % 1 === 0)
          let color = '#3b82f6'; // blue (1/16 in German description, but actually 1/4 beat)
          let height = 'h-3';
          let mt = 'mt-5.5';
...
content = content.replace(/const renderTimeline = \(\) => \{[^]*?    \);\n  \};/, timelineReplacement.trim());


// 3. Audio/Video Tags and keyboard events
const audioVideoReplacement = `
...
// we need to insert the audio/video ref logic. We already have the refs in the file.
// let's replace the existing useEffects that handle playback with the new ones.
// Currently there is:
// const audioRef = React.useRef<HTMLAudioElement | null>(null);
// const videoRef = React.useRef<HTMLVideoElement | null>(null);

// Find where these are and replace the block after them until handleWheel

const refRegex = /const audioRef = React\.useRef<HTMLAudioElement \| null>\(null\);\n  const videoRef = React\.useRef<HTMLVideoElement \| null>\(null\);[^]*?const handleWheel = /;

content = content.replace(refRegex, audioVideoReplacement + '\n  const handleWheel = ');

// 4. Inject <audio> and <video> tags into Main Area playfield
const playfieldReplacement = `
{/* BG Image / Video */}
              {videoUrl ? (
...
content = content.replace(/\{\/\* BG Image \*\/\}[\s\S]*?<div className="absolute inset-0 bg-\[#1a1c20\]\/40" \/>/, playfieldReplacement.trim());


// 5. Replace Slider/Spinner rendering
const hitObjectsRenderReplacement = `
              {/* Render Hit Objects */}
                  ...
                  if (obj.type === 'slider') {
                    ...
                    return (
                      <div key={obj.id} className="absolute inset-0" style={{ opacity }}>
                        <svg viewBox="0 0 512 384" className="w-full h-full absolute inset-0 drop-shadow-xl" preserveAspectRatio="none">
                        ...
                        >
                          <div 
                            className={\`w-[80px] h-[80px] rounded-full border-[5px] flex items-center justify-center shadow-[0_0_20px_rgba(0,0,0,0.5)] bg-black/40 backdrop-blur-sm pointer-events-auto cursor-pointer \${isSelected ? 'ring-4 ring-[#fdb438]' : ''}\`}
                    ...
                    return (
                      ...
                      >
                         <div 
                           className={\`w-[300px] h-[300px] rounded-full border-[10px] border-white/20 flex items-center justify-center bg-black/20 pointer-events-auto cursor-pointer \${isSelected ? 'ring-4 ring-[#fdb438]' : ''}\`}
                           onPointerDown={(e) => handleObjectPointerDown(e, obj)}
                         >
...
content = content.replace(/\{\/\* Render Hit Objects \*\/\}[\s\S]*?<\/div>\n            <\/div>/, hitObjectsRenderReplacement.trim() + '\n            </div>');

fs.writeFileSync('src/components/BeatmapEditor.tsx', content);
